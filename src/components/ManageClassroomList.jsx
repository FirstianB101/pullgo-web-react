...
const ManageClassroomList = ({
            ...
            const element = (
                <div className="div__classroom_list_menu">
                    <li key={joinedClassroomList[i].id}
                        onClick={() => history.push(`/teacher/manage_exam/${joinedClassroomList[i].id}`)}>
                        <span className="classroom_name">
                            {classroomName} 반
                        </span>
                        <span className="academy_name">{academyName} 학원</span>
    ...
    return (
        <div className="manage_classroom_list">
            <ul className="ul__classroom_list">{showClassroomListItems()}</ul>
        </div>
    );
};
